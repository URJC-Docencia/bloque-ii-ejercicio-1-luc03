...
public class LinkedTree<E> implements NAryTree<E> {
    ...
    private static <E> void checkpositionofChildren(int n, LinkedTree<E>.TreeNode<E> parent){
        if(n < 0 || n > parent.getChildren().size()){
            throw new RuntimeException("Te position is invalid");
        }
    }
    private TreeNode checkPosition(Position<E> p){
    ...
    public Position<E> add(E element, Position<E> p, int n) {
        TreeNode<E> parent = checkPosition(p);
        checkpositionofChildren(n,parent);
        TreeNode<E> newNode = new TreeNode<E>(element,parent);
